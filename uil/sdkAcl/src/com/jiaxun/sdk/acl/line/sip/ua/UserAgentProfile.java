...
public class UserAgentProfile extends Configure {
	...
	/**
	 * Automatic call Forward time in seconds; time<0 corresponds to no auto
	 * Forward mode.
	 */
	public int Forward_time = -1;
	/**
	...
	/** Video port */
	public static int video_port = 21070;
	/** Video avp */
	...
	private void init() {
		...
		if (redirect_to != null && redirect_to.equalsIgnoreCase(Configure.NONE))
			redirect_to = null;
		if (Forward_to != null && Forward_to.equalsIgnoreCase(Configure.NONE))
			Forward_to = null;
	...
	protected void parseLine(String line) {
		...
		if (attribute.equals("contact_url")) {
			...
			Log.error("UserAgentProfile.parseLine", "contact_url:"+contact_url);
			return;
		}
		...
		if (attribute.equals("Forward_time")) {
			Forward_time = par.getInt();
			return;
		}
		...
		if (attribute.equals("Forward_to")) {
			Forward_to = par.getRemainingString().trim();
			return;
		}
    ...
    public int getAudio_port() {
        audio_port += 2;
        audio_port = (audio_port - 21000) % 100 + 21000;
    ...
    public int getVideo_port() {
        video_port += 2;
        video_port = (video_port - 21070) % 100 + 21070;
        
        return video_port;
    }

    public void setVideo_port(int audio_port) {
        this.video_port = audio_port;
    }
