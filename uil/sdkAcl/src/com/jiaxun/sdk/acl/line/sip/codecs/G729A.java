...
public class G729A extends CodecBase implements Codec
{
    ...
    void load()
    {
        try
        {
            System.loadLibrary("G729A_jni");
            super.load();
        }
        catch (Exception ex)
        {
            Log.exception("G729A", ex);
        }
    }

    public native static long initEncoder();

    public native static void encoder(long handle, short[] lin, int offset, byte[] bitstream, int packetCounts);

    public native static void freeEncoder(long handle);

    public native static long initDecoder();

    public native static void decoder(long handle, byte[] bitstream, short[] synth_short, int i);
