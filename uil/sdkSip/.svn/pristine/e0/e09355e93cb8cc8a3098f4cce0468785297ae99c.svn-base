...
import org.zoolu.attribute.ChangeRequest;
import org.zoolu.attribute.ChangedAddress;
import org.zoolu.attribute.ErrorCode;
import org.zoolu.attribute.MappedAddress;
import org.zoolu.attribute.MessageAttribute;
import org.zoolu.attribute.MessageAttributeException;
import org.zoolu.attribute.MessageAttributeParsingException;
import org.zoolu.header.MessageHeader;
import org.zoolu.header.MessageHeaderParsingException;
import org.zoolu.tools.UtilityException;
	...
	private boolean test1() throws UtilityException, SocketException, UnknownHostException, IOException, MessageAttributeParsingException, MessageHeaderParsingException {
		...
		while (true) {
			try {
				...
				Log.info(TAG, "Test 1: Binding Request sent.");
				...
				if (ec != null) {
					...
					Log.info(TAG, "Message header contains an Errorcode message attribute.");
					return false;
				}
				if ((ma == null) || (ca == null)) {
					...
					Log.info(TAG, "Response does not contain a Mapped Address or Changed Address message attribute.");
					return false;
				} else {
					...
					if ((ma.getPort() == socketTest1.getLocalPort()) && (ma.getAddress().getInetAddress().equals(socketTest1.getLocalAddress()))) {
						Log.info(TAG, "Node is not natted.");
						nodeNatted = false;
					} else {
						Log.info(TAG, "Node is natted.");
					}
					return true;
				}
			} catch (SocketTimeoutException ste) {
				if (timeSinceFirstTransmission < 3000) { // was 7900
					Log.info(TAG, "Test 1: Socket timeout while receiving the response.");
					...
					Log.info(TAG, "Test 1: Socket timeout while receiving the response. Maximum retry limit exceed. Give up.");
					di.setBlockedUDP();
					Log.info(TAG, "Node is not capable of UDP communication.");
					return false;
				}
			} 
		}
	...
	private boolean test2() throws UtilityException, SocketException, UnknownHostException, IOException, MessageAttributeParsingException, MessageAttributeException, MessageHeaderParsingException {
		...
		while (true) {
			try {
				...
				Log.info(TAG, "Test 2: Binding Request sent.");
				...
				if (!nodeNatted) {
					di.setOpenAccess();
					Log.info(TAG, "Node has open access to the Internet (or, at least the node is behind a full-cone NAT without translation).");
				} else {
					di.setFullCone();
					Log.info(TAG, "Node is behind a full-cone NAT.");
				}
				return false;
			} catch (SocketTimeoutException ste) {
				if (timeSinceFirstTransmission < 7900) {
					Log.info(TAG, "Test 2: Socket timeout while receiving the response.");
					timeSinceFirstTransmission += timeout;
					int timeoutAddValue = (timeSinceFirstTransmission * 2);
					if (timeoutAddValue > 1600) timeoutAddValue = 1600;
					timeout = timeoutAddValue;
				} else {
					Log.info(TAG, "Test 2: Socket timeout while receiving the response. Maximum retry limit exceed. Give up.");
					if (!nodeNatted) {
						di.setSymmetricUDPFirewall();
						Log.info(TAG, "Node is behind a symmetric UDP firewall.");
						return false;
					} else {
						// not is natted
	...
	private boolean test1Redo() throws UtilityException, SocketException, UnknownHostException, IOException, MessageAttributeParsingException, MessageHeaderParsingException{
		...
		while (true) {
			...
			try {
				...
				DatagramPacket send = new DatagramPacket(data, data.length);
				socketTest1.send(send);
				Log.info(TAG, "Test 1 redo with changed address: Binding Request sent.");
				...
				if (ma2 == null) {
					...
					Log.info(TAG, "Response does not contain a Mapped Address message attribute.");
					return false;
				} else {
					if ((ma.getPort() != ma2.getPort()) || (!(ma.getAddress().getInetAddress().equals(ma2.getAddress().getInetAddress())))) {
						di.setSymmetric();
						Log.info(TAG, "Node is behind a symmetric NAT.");
						return false;
					}
				}
				return true;
				...
				if (timeSinceFirstTransmission < 7900) {
					Log.info(TAG, "Test 1 redo with changed address: Socket timeout while receiving the response.");
					timeSinceFirstTransmission += timeout;
					int timeoutAddValue = (timeSinceFirstTransmission * 2);
					if (timeoutAddValue > 1600) timeoutAddValue = 1600;
					timeout = timeoutAddValue;
				} else {
					Log.info(TAG, "Test 1 redo with changed address: Socket timeout while receiving the response.  Maximum retry limit exceed. Give up.");
					return false;
				}
			}
		}
	...
	private void test3() throws UtilityException, SocketException, UnknownHostException, IOException, MessageAttributeParsingException, MessageAttributeException, MessageHeaderParsingException {
		...
		while (true) {
			try {
				...
				DatagramPacket send = new DatagramPacket(data, data.length);
				sendSocket.send(send);
				Log.info(TAG, "Test 3: Binding Request sent.");
				...
				if (nodeNatted) {
					...
					Log.info(TAG, "Node is behind a restricted NAT.");
					return;
				}
			} catch (SocketTimeoutException ste) {
				if (timeSinceFirstTransmission < 7900) {
					Log.info(TAG, "Test 3: Socket timeout while receiving the response.");
					timeSinceFirstTransmission += timeout;
					int timeoutAddValue = (timeSinceFirstTransmission * 2);
					if (timeoutAddValue > 1600) timeoutAddValue = 1600;
					timeout = timeoutAddValue;
				} else {
					Log.info(TAG, "Test 3: Socket timeout while receiving the response. Maximum retry limit exceed. Give up.");
					di.setPortRestrictedCone();
					Log.info(TAG, "Node is behind a port restricted NAT.");
					return;
				}
			}
		}
