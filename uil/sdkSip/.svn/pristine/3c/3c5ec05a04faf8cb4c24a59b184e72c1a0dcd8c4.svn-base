...
public class SubscriberDialog extends Dialog implements
	...
	private static String TAG = "SubscriberDialog";
	...
	public void subscribe(String target, String subscriber, String contact,
            int expires, String body) throws Exception {
        Log.info(TAG, "subscribe::target:" + target + " subscriber:"
                + subscriber + " contact:" + contact + " id:" + id
                + " expires:" + expires + " body:" + body);
        ...
        if(body != null)
        {
            String content_type = "application/psap+xml";
	...
	public void subscribe(String target, String subscriber, String contact,
	    ...
	    Log.info(TAG, "subscribe::target:" + target + " subscriber:"
                + subscriber + " contact:" + contact + " id:" + id
                + " expires:" + expires);
	    subscribe(target, subscriber, contact, expires, null);
	}
	 ...
	 * @throws Exception 
	 */
	public void subscribe(Message req) throws Exception {
	    Log.info(TAG, "subscribe");
		if (statusIs(D_TERMINATED)) {
		    Log.info(TAG, "subscription already terminated: request aborted");
			return;
		}
		// else
		if (statusIs(D_INIT)) {
	...
	public void onTransFailureResponse(TransactionClient tc, Message resp) {
	    Log.info(TAG, "onTransFailureResponse");
...
//		changeStatus(D_TERMINATED);
	...
	public void onReceivedMessage(SipProvider sip_provider, Message msg) throws Exception {
		...
		if (statusIs(D_TERMINATED)) {
		    Log.info(TAG, "subscription already terminated: message discarded");
			return;
		}
		// else
...
//					changeStatus(D_TERMINATED);
		    ...
		    Log.info(TAG, "message is not a NOTIFY: message discarded");
		}
	}
