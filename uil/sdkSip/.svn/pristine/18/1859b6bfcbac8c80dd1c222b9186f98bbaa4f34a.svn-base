...
public class InviteTransactionServer extends TransactionServer
{
     ...
     * @throws Exception 
     */
    public InviteTransactionServer(SipProvider sip_provider, Message invite, InviteTransactionServerListener listener) throws Exception
    {
        super(sip_provider);
     ...
     * @throws Exception 
     */
    public InviteTransactionServer(SipProvider sip_provider, Message invite, boolean auto_trying, InviteTransactionServerListener listener) throws Exception
    {
        super(sip_provider);
        request = new Message(invite);
        init(listener, request.getTransactionId(), request.getConnectionId());
    ...
    public void listen()
    {
        if (statusIs(STATE_IDLE))
        {
            ...
            sip_provider.addSipProviderListener(new TransactionIdentifier(SipMethods.OPTIONS), this);
        }
    }

    /** Sends a response message 
     * @throws Exception */
    public void respondWith(Message resp) throws Exception
    {
    	Log.info("InviteTransactionServer", "respondWith");
        response = resp;
        ...
        if (code >= 300 && code < 700 && (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING)))
        {
            ...
            {
                Log.info("InviteTransactionServer.respondWith", 
                        "No retransmissions for reliable transport (" + connection_id + ")");
                onTimeout(end_to);
            }
        }
    }
    ...
    /**
     * Method derived from interface ServiceListener. It's fired from the
    ...
    public void onReceivedMessage(SipProvider provider, Message msg) throws Exception
    {
        if (msg.isRequest())
        {
            ...
            Log.info("InviteTransactionServer.onReceivedMessage", "req_method:" + req_method);
            ...
            if (req_method.equals(SipMethods.INVITE))
            {
                ...
                {
                    PerfTestHelper.CALLPREF.setReceiveInvite(System.currentTimeMillis());
                }
                // --------------------------------呼叫发起性能测试日志记录
                // End----------------------------------------------
                ...
                if (statusIs(STATE_WAITING))
                {
                    ...
                    if (auto_trying)
                    {
                        ...
                        if (CommonConfigEntry.TEST_CALL)
                        {
                            PerfTestHelper.CALLPREF.setSend100Trying(System.currentTimeMillis());
                        }
                        // --------------------------------呼叫发起性能测试日志记录
                        // End----------------------------------------------
        ...
        else if(msg.isResponse())
        {
            ...
            {//心跳消息
                if (transaction_listener != null)
                    ((InviteDialog) transaction_listener).onReceivedMessage(sip_provider, msg);
                return;
            }
        }
    }
    ...
    public void onTimeout(Timer to)
    {
        try
        {
            ...
            {
                Log.error("InviteTransactionServer.onTimeout", "retransmission_to");
            ...
            {
                Log.error("InviteTransactionServer.onTimeout", "end_to");
                retransmission_to.halt();
                sip_provider.removeSipProviderListener(transaction_id);
                changeStatus(STATE_TERMINATED);
//                transaction_listener = null;
            ...
            {
                Log.error("InviteTransactionServer.onTimeout", "clearing_to");
                sip_provider.removeSipProviderListener(transaction_id);
                changeStatus(STATE_TERMINATED);
//                transaction_listener = null;
            }
        ...
        {
            Log.exception("InviteTransactionServer.onTimeout", e);
        }
    }
    ...
    {
        Log.error("InviteTransactionServer", "terminate");
        retransmission_to.halt();
        clearing_to.halt();
        end_to.halt();
