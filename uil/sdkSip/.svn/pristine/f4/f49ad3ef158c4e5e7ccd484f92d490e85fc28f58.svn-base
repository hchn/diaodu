...
import org.zoolu.sip.header.TerminateHeader;
...
public abstract class BaseMessageFactory {
     ...
     *            the message body
     */
    public static Message createRequest(Dialog dialog, String method, NameAddress to, 
            String body) {
        ...
        if (!SipMethods.isAck(method) && !SipMethods.isCancel(method))
            dialog.incLocalCSeq();
        if(SipMethods.isBye(method))//20121031 用于当挂断操作快于第二个invite消息时
        {
            dialog.incLocalCSeq();
        }
	...
	public static Message createRequest(Dialog dialog, String method,
		...
		return createRequest(dialog, method, to, body);
	}
	...
	public static Message createInviteRequest(SipProvider sip_provider,
			...
			NameAddress contact, String body, String icsi, int priority, 
			String funcNumber, String callid, boolean activeGroup) {
		int cseq = SipProvider.pickInitialCSeq();
		String local_tag = SipProvider.pickTag();
		// String branch=SipStack.pickBranch();
		if (contact == null)
			contact = from;
		Message request = createRequest(sip_provider, SipMethods.INVITE, request_uri, to,
				from, contact, callid, cseq, local_tag, null, null, body, icsi);

		// the priority values should in 0~4
        if (priority >= 0 && priority <= 4) {
            ResourcePriorityHeader rph = new ResourcePriorityHeader(priority);
            request.setResourcePriorityHeader(rph);
        }

        if(funcNumber != null && !funcNumber.equals("")) {
            UserToUserHeader uui = new UserToUserHeader(funcNumber);
            uui.setEncoding("hex");
            uui.setContent("gsmr-uui");
            request.setUserToUserHeader(uui);
        }
        
        if(activeGroup)
        {//加入呼叫中组
            request.setHeader(new Header(SipHeaders.Extra_Methord, "activeGroup"));
        }
	...
	public static Message createByeRequest(Dialog dialog,int tag) {
		Message msg = createRequest(dialog, SipMethods.BYE, null);
		if(tag == 1)
		{
			msg.setTerminateHeader(new TerminateHeader("terminate=1"));
		}
    ...
    /**
     * Creates a new NOTIFY request.
     * <p>
     * If contact is null, set contact as star * (register all)
     */
    public static Message createNotifyRequest(SipProvider sip_provider, NameAddress to, NameAddress from,
                                              NameAddress contact, String event, String id, String content_type, String body) {
        SipURL to_url = to.getAddress();
        SipURL registrar = new SipURL(to_url.getHost(), to_url.getPort());
        String via_addr = sip_provider.getViaAddress();
        int host_port = sip_provider.getPort();
        ...
        String local_tag = SipProvider.pickTag();
        // String branch=SipStack.pickBranch();
        Message req = createRequest(SipMethods.NOTIFY, registrar, to, from,
                contact, proto, via_addr, host_port, rport, call_id, cseq,
                local_tag, null, null, null, null, null);						// modified by mandrajg
