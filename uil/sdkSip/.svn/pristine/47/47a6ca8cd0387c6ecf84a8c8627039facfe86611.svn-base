 ...
 * <p />
 * MessageFactory is used to create SIP messages (requests and responses). <br />
...
public class MessageFactory extends org.zoolu.sip.message.BaseMessageFactory {
	...
	public static Message createNotifyRequest(SipProvider sip_provider,
			...
			String event, String id, String content_type, String body) {
		...
		Message req = createRequest(SipMethods.NOTIFY, registrar, to, from,
				...
				local_tag, null, null, null, null, null); // modified by
															// mandrajg
	...
	public static Message createMessageRequest(SipProvider sip_provider,
		...
		String local_tag = SipProvider.pickTag();
		// String branch=SipStack.pickBranch();
		Message req = createRequest(SipMethods.MESSAGE, registrar, to, from,
				contact, proto, via_addr, host_port, rport, call_id, cseq,
				local_tag, null, null, null, null, null); // modified by
															// mandrajg
	...
	/** Creates a new INFO request (RFC2976) within a dialog */
	public static Message createInfoRequest(SipProvider sip_provider,
			NameAddress server, NameAddress to, NameAddress from,
			NameAddress contact, String content_type, String body) {
		SipURL to_url = to.getAddress();
		SipURL svr_url = server.getAddress();
		SipURL registrar = new SipURL(svr_url.getHost(), svr_url.getPort());
		String via_addr = sip_provider.getViaAddress();
		int host_port = sip_provider.getPort();
		boolean rport = sip_provider.isRportSet();
		String proto;
		...
		String local_tag = SipProvider.pickTag();
		// String branch=SipStack.pickBranch();
		Message req = createRequest(SipMethods.INFO, svr_url, to, from,
				contact, proto, via_addr, host_port, rport, call_id, cseq,
				local_tag, null, null, null, null, null); // modified by
															// mandrajg
