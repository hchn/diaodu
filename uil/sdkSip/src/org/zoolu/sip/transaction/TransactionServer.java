...
public class TransactionServer extends Transaction {
	...
	public void respondWith(Message resp) throws Exception {
		Log.info("TransactionServer", "respondWith");
		response = resp;
		if (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING)) {
			...
			if (code >= 200 && code < 700) {
				...
				if (true || connection_id == null) // modified
				    ...
				    Log.info("TransactionServer", "clearing_to=0 for reliable transport");
	...
	public void onReceivedMessage(SipProvider provider, Message msg) throws Exception {
	    Log.info("TransactionServer", "onReceivedMessage");
		...
		if (msg.isRequest()) {
			...
			if (statusIs(STATE_PROCEEDING) || statusIs(STATE_COMPLETED)) { // retransmission of the last response
				Log.info("TransactionServer", "response retransmission");
				try
                {
                    sip_provider.sendMessage(response, connection_id);
                }
                ...
                {
                    Log.exception("TransactionServer", e);
                }
				return;
			}
		}
	...
	public void onTimeout(Timer to) {
	    Log.error("TransactionServer", "onTimeout");

		try {
			if (to.equals(clearing_to)) {
			    Log.error("TransactionServer", "Clearing timeout expired");
				sip_provider.removeSipProviderListener(transaction_id);
				changeStatus(STATE_TERMINATED);
				transaction_listener = null;
			}
		} catch (Exception e) {
		    Log.exception("TransactionServer.onTimeout", e);
		}
	}
