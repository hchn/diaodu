...
public class TransactionClient extends Transaction {
	...
	private static String TAG = "TransactionClient";
	...
	void init(TransactionClientListener listener,
		...
		Log.info(TAG, "init::transaction_id:" + String.valueOf(transaction_id));
	}

	/** Starts the TransactionClient and sends the transaction request. 
	 * @throws Exception */
	public void request() throws Exception {
	    Log.info(TAG, "sends the transaction request.");
	...
	public void onReceivedMessage(SipProvider provider, Message msg) throws Exception {
		...
		if (msg.isResponse()) 
		{
			...
			{
//				if((msg.getMethodId().toString().equals("BYE") || msg.getMethodId().toString().equals("CANCEL")))
//				{//Cancel和Bye时收到的消息不是200时应忽略 直到服务器处理为止 20121123
//					return;
//				}
				retransmission_to.halt();
				transaction_to.halt();
				...
				if (true || connection_id == null) // modified
				    ...
				    Log.info(TAG, "clearing_to=0 for reliable transport");
	...
	public void onTimeout(Timer to) {
		try {
			if (to.equals(retransmission_to)
					&& (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING))) {//周期重传
			...
			if (to.equals(clearing_to)) {//重传成功清除定时器
			    Log.info(TAG, "onTimeout::clearing_to");
				retransmission_to.halt();
				transaction_to.halt();
				sip_provider.removeSipProviderListener(transaction_id);
				changeStatus(STATE_TERMINATED);
