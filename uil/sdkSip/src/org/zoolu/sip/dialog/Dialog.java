...
public abstract class Dialog extends DialogInfo implements SipProviderListener {
	 ...
	 * @throws Exception */
	abstract public void onReceivedMessage(SipProvider provider, Message message) throws Exception;
	...
	protected void changeStatus(int newstatus) {
		status = newstatus;
		Log.info("Dialog", "changed dialog state: " + getStatus());
	...
	public void update(int side, Message msg) {
		Log.info("Dialog.update", "side:" + side);
		if (isTerminated()) {
		    Log.error("Dialog", "trying to update a terminated dialog: do nothing.");
			return;
		}
		// else
		...
		if (dialog_id == null || !dialog_id.equals(new_id)) {
			if (dialog_id != null && sip_provider != null
				...
				sip_provider.removeSipProviderListener(dialog_id);
			dialog_id = new_id;
			Log.info("Dialog.update", "new dialog id: " + dialog_id);
	...
	protected final boolean verifyThat(boolean expression, String str) {
		if (!expression) {
			if (str == null || str.length() == 0)
				Log.error("Dialog.verifyThat", "expression check failed. ");
			else
			    Log.error("Dialog.verifyThat", str);
		}
		return expression;
	}
