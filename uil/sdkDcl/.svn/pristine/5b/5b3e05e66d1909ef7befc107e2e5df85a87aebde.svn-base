...
public class CharacterParser {
	...
	/** * 词组解析 * * @param chs * @return */
    public String[] getSellings(String chs) {
        if (TextUtils.isEmpty(chs))
        {
            return null;
        }
        String key, value;
        strs = new String[chs.length()];
        for (int i = 0; i < chs.length(); i++) {
            key = chs.substring(i, i + 1);
            if (key.getBytes().length >= 2) {
                value = (String) convert(key);
	...
	public String getSpelling() {
	    ...
	    {
	        return convertSelling(resource);
	    }
	...
	public String[] getSpellings() {
	    if(buffer == null || buffer.length() == 0)
            convertSelling(resource);
        return strs;
    }
	
	/** * 词组解析 * * @param chs * @return */
    public String convertSelling(String chs) {
        if (TextUtils.isEmpty(chs))
        {
            return chs;
        }
