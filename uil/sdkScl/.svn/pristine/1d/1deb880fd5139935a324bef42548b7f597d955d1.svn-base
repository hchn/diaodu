...
public class RtpPacket implements Cloneable {
	...
	public int getHeaderLength() {
		...
		{
		    if(hasExtension())
		        return 12 + 4 * getCscrCount() + 8;// 暂时固定加入8个字节扩展位
		    else
    ...
    public int getExtTag()
    {
        if(hasExtension())
        {
            return getInt(packet, 16, 20);//暂时获取扩展位中4个字节长度的tag值
        }
        return 0;
    }
    ...
    public RtpPacket clone()
    {
        ...
        try
        {
            rtpPacket = (RtpPacket) super.clone();
            rtpPacket.packet = this.packet.clone();
        }
        catch (CloneNotSupportedException e)
        {
            Log.exception("RtpPacket", e);
        }
        return rtpPacket;
    }
}
