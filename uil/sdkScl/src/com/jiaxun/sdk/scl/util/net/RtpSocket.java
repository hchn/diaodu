...
package com.jiaxun.sdk.scl.util.net;
...
public class RtpSocket {
	...
	public void receive(RtpPacket rtpp) throws IOException {
		...
		if(test)
        {
		    long currentReceive = System.currentTimeMillis();
		    long interval = currentReceive - lastReceive;
		    if (interval >= 80) {// 大时间间隔
		        Log.info("RtpSocket", "receive seqn: " + rtpp.getSequenceNumber() + " interval:" + interval);
		    }
		    lastReceive = currentReceive;
        }
	}
	...
	public void send(RtpPacket rtpp) throws IOException {
		...
		if(test)
		{
		    long currentSend = System.currentTimeMillis();
		    long interval = currentSend - lastSendTime;
		    if (interval >= 80) {// 大时间间隔
		        Log.info("RtpSocket", "send seqn: " + rtpp.getSequenceNumber() + " interval:" + interval);
		    }
		    lastSendTime = currentSend;
		}
	}
